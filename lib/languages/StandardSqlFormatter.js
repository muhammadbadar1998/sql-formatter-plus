...
const reservedTopLevelWords = ['ADD', 'AFTER', 'ALTER COLUMN', 'ALTER TABLE', 'DELETE FROM', 'EXCEPT', 'FETCH FIRST', 'FROM', 'GROUP BY', 'GO', 'HAVING', 'INSERT INTO', 'INSERT', 'LIMIT', 'MODIFY', 'ORDER BY', 'SELECT', 'SET CURRENT SCHEMA', 'SET SCHEMA', 'SET', 'UPDATE', 'VALUES', 'WHERE'];
const reservedTopLevelWordsNoIndent = ['INTERSECT', 'INTERSECT ALL', 'MINUS', 'UNION', 'UNION ALL'];
const reservedNewlineWords = ['AND', 'CROSS APPLY', 'CROSS JOIN', 'ELSE', 'INNER JOIN', 'JOIN', 'LEFT JOIN', 'LEFT OUTER JOIN', 'OR', 'OUTER APPLY', 'OUTER JOIN', 'RIGHT JOIN', 'RIGHT OUTER JOIN', 'WHEN', 'XOR', 'INTERSECT', 'INTERSECT ALL', 'MINUS', 'UNION', 'UNION ALL'];
let tokenizer;
export default class StandardSqlFormatter {
  ...
  format(query) {
    ...
    const formattedQuery = new Formatter(this.cfg, tokenizer).format(query); // Add newlines before any JOIN keywords

    const joinKeywords = ['JOIN', 'INNER JOIN', 'LEFT JOIN', 'RIGHT JOIN', 'OUTER JOIN', 'ON']; // Replace occurrences of JOIN with two newlines before

    const finalFormattedQuery = formattedQuery.replace(new RegExp(`(${joinKeywords.join('|')})`, 'gi'), // Case insensitive match for JOIN keywords
