...
var reservedNewlineWords = ['AND', 'CROSS APPLY', 'CROSS JOIN', 'ELSE', 'INNER JOIN', 'JOIN', 'LEFT JOIN', 'LEFT OUTER JOIN', 'OR', 'OUTER APPLY', 'OUTER JOIN', 'RIGHT JOIN', 'RIGHT OUTER JOIN', 'WHEN', 'XOR', 'INTERSECT', 'INTERSECT ALL', 'MINUS', 'UNION', 'UNION ALL'];
var tokenizer;
...
function () {
  ...
  _createClass(StandardSqlFormatter, [{
    ...
    value: function format(query) {
      ...
      var formattedQuery = new _Formatter["default"](this.cfg, tokenizer).format(query); // Add newlines before any JOIN keywords

      var joinKeywords = ['JOIN', 'INNER JOIN', 'LEFT JOIN', 'RIGHT JOIN', 'OUTER JOIN', 'ON']; // Replace occurrences of JOIN with two newlines before

      var finalFormattedQuery = formattedQuery.replace(new RegExp("(".concat(joinKeywords.join('|'), ")"), 'gi'), // Case insensitive match for JOIN keywords
